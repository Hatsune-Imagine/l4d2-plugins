	...
	if (IsValidClient(attacker)) {
		if (IsClientInGame(attacker)) {
			switch (GetClientTeam(attacker)) {
				case 2: {
					if (!idleplayer)
						CPrintToChatAll("{blue}%N {default}死亡 {green}| {olive}%N {green}| {default}伤害:{olive}%.1f", victim, attacker, damage);
					else
						CPrintToChatAll("{blue}%N {default}死亡 {green}| {default}[{olive}闲置{default}]{blue}%N {green}| {default}伤害:{olive}%.1f", victim, idleplayer, damage);
				}

				case 3: {
					if (IsFakeClient(attacker))
						CPrintToChatAll("{olive}%N {default}死亡 {green}| {red}%N {green}| {default}伤害:{olive}%.1f", victim, attacker, damage);
					else
						CPrintToChatAll("{olive}%N {default}死亡 {green}| {red}%N{default}({olive}%s{default}) {green}| {default}伤害:{olive}%.1f", victim, attacker, g_sZombieClass[GetEntProp(attacker, Prop_Send, "m_zombieClass") - 1], damage);
				}
			}
		}
	}
		...
		if (!idleplayer)
			CPrintToChatAll("{blue}%N {default}死亡 {green}| {olive}%s {green}| {default}伤害:{olive}%.1f", victim, cls, damage);
		else
			CPrintToChatAll("{default}[{olive}闲置{default}]{blue}%N {default}死亡 {green}| {olive}%s {green}| {default}伤害:{olive}%.1f", idleplayer, cls, damage);
	}
}
...
void Event_WitchHarasserSet(Event event, const char[] name, bool dontBroadcast) {
	...
	switch (GetClientTeam(client)) {
		case 2: {
			...
			if (!idleplayer)
				CPrintToChatAll("{blue}%N {default}惊扰了 {olive}witch", client);
			else
				CPrintToChatAll("{default}[{olive}闲置{default}]{blue}%N {default}惊扰了 {olive}witch", idleplayer);
		}

		case 3:
			CPrintToChatAll("{red}%N {default}惊扰了 {olive}witch", client);
	}
}
...
void NextFrame_LastLife(int client) {
	if ((client = GetClientOfUserId(client)) && IsClientInGame(client) && GetClientTeam(client) == 2 && IsPlayerAlive(client) && GetEntProp(client, Prop_Send, "m_currentReviveCount") >= g_iSurvivorMaxInc) {
		...
		for (int i = 1; i <= MaxClients; i++) {
			...
			if (!idleplayer)
				CPrintToChat(i, "{blue}%N {default}进入 {olive}黑白状态", client);
			else
				CPrintToChat(i, "{default}[{blue}闲置{default}]{blue}%N {default}进入 {olive}黑白状态", idleplayer);
