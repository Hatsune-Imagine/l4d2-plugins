...
#define PLUGIN_AUTHOR			"DDRKhat, Marcus101RR, Merudo, Lux, Shadowysn, sorallll, HatsuneImagine"
#define PLUGIN_DESCRIPTION		"coop"
#define PLUGIN_VERSION			"1.11.8a"
...
public void OnPluginStart() {
	...
	g_cJoinCompetitive.AddChangeHook(CvarChanged_General);
...
Action cmdJoinTeam2(int client, int args) {
	if (g_iJoinCompetitive == 0 && IsVersusOrScavenge())
		return Plugin_Handled;
...
void GeCvars_General() {
	...
	g_iJoinCompetitive =	g_cJoinCompetitive.IntValue;
...
bool IsVersusOrScavenge() {
	char gmode[32];
	GetConVarString(FindConVar("mp_gamemode"), gmode, sizeof(gmode));
	return (StrContains(gmode, "versus") > -1 || StrContains(gmode, "scavenge") > -1);
}
