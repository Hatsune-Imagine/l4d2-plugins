...
{
	hCvarPrintText = CreateConVar("l4d2_slowdown_print_text", "1", "Whether print hint text in chat box or not.");
...
public void OnMapStart() {
	if (L4D_IsVersusMode()) {
		...
		ResetConVar(FindConVar("z_tank_damage_slow_min_range"));
		ResetConVar(FindConVar("z_tank_damage_slow_max_range"));
	}
}
...
public void TankSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (!tankInPlay) {
		tankInPlay = true;
		if (hCvarPrintText.BoolValue && fSurvWaterSpeedDuringTank > 0.0) {
